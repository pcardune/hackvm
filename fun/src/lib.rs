...
use crate::ast::{FieldDecl, LetStatement, MethodDecl, Statement};

pub fn parse_module(input: &str) -> Result<Module> {
    let mut module = Module { classes: vec![] };
        ...
        match pair.as_rule() {
            Rule::file => {
                for pair in pair.into_inner() {
                    match pair.as_rule() {
                        Rule::class_decl => module.classes.push(parse_class_decl(pair)?),
                        Rule::EOI => {
                            break;
                        }
...
fn parse_expr(pair: Pair<Rule>) -> Result<Expression> {
    let mut pairs = pair.into_inner();
    let term = parse_term(pairs.next().expect("Expression unexpectedly has no terms"))?;
...
mod tests {
    ...
    fn test_expr() {
        ...
        assert_eq!(parse("0").term().as_number(), Some(0));
        assert_eq!(parse("true").term().as_bool(), Some(true));
        assert_eq!(parse("\"foo\"").term().as_string(), Some("foo"));
